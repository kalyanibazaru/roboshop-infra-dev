...
resource "aws_autoscaling_group" "catalogue" {
  ...
  triggers = ["launch_template"]
    ...
    #Here Rollingupdate purpose is, without application downtime, one new instance is created- when it is up then the old instance will be deleted. similarlly when 2nd new instance is created, when it is up  and then 2nd old instance will be deleted. automally new instances will be created.
